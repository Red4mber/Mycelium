...
use crate::routes::authenticated::query_operator_by_id;
...
pub fn generate_token(
    ...
    let claim = Claims {
        ...
        exp: (now + SETTINGS.tokens.ttl).timestamp() as usize,
        // typ,
    };

    encode(&Header::new(Algorithm::EdDSA), &claim, encoding_key).map_err(|_| Error::TokenCreation)
}
...
///
/// Reads the `Authorization` Header to recover the token \
...
pub async fn auth_middleware(
    ...
    let claims = validate_token(token, &state.operator_keys.decoding_key)?;

    let operator = query_operator_by_id(&claims.sub, &state.db)
        .await
